...
3. That sequence is added as an **undistorted_plate** representation on the **same matchmove product** (no separate “plate” product).
...
5. AYON publishes everything into the **same version folder** as the matchmove (e.g. `.../matchmoveMain/v014/`), so the script and the plate EXRs live together.

Result: one publish produces the matchmove script and the undistorted plate in the right place, with the script pointing at the plate by full path and correct AYON naming.
...
- The undistorted EXRs are **not** a separate AYON product. They are a **representation** (`undistorted_plate`) on the **matchmove** product. AYON publishes them into the **same version folder** as the matchmove (e.g. `.../matchmoveMain/v014/`). So you get one version folder containing both the Maya script and the plate EXRs, and the script path we write matches that layout.
...
2. **Extract (order: plate first, then Maya)**
   - **Extract Undistorted Plate** (runs first):
     ...
     - Adds the `undistorted_plate` representation and sets `instance.data["plate_base_name"]` for the Maya extractor.
...
3. **Integrate**
   - AYON integrates the matchmove product (script + undistorted_plate representation) into the version folder.
